...
def eps_rnk(ffv,ccv,eps):
    return eps_sort(ffv,ccv,eps)

def lex_rnk(ffv,ccv,eps):
    return eps_sort(ffv,ccv,0)

#####
# gradient-based repair

def repair(y,max_reps,fevals,gv,hv,convio,objFun,lower_bounds,upper_bounds,delt,dim,g): 
    reps=1  
    new_y=y.copy()
    ...
    if g % dim == 0 and np.random.rand(1) <= 0.2: 
        while convio > 0 and reps <= max_reps:
            ...
            convio = np.sum([np.sum(gv*(gv>0)), np.sum(np.abs(hv)*(np.abs(hv)>delt))])
            fevals = fevals + dim +1
            reps=reps+1
            new_y = new_mutant
    return new_y
...
def MAES(yInit,mu,lam,sigma,maxIter,budget,objFun,lower_bounds,upper_bounds,bch,rep,rnk):
...
# H. -G. Beyer and B. Sendhoff, "Simplify Your Covariance Matrix Adaptation Evolution Strategy," 
    ...
    # some repair parameters (if applicable)
    delt = 10**(-4)
    max_reps=3
    ...
    while not termination:
        ...
        for k in range(lam):
            ...
            if newpop_cv[k] > 0:
                newpop_y[:,k]=repair(newpop_y[:,k],max_reps,fevals,gv,hv,newpop_cv[k],objFun,lower_bounds,upper_bounds,delt,dim,g)
                newpop_f[k], gv, hv = objFun(newpop_y[:,k])
                newpop_cv[k] = np.sum([np.sum(gv*(gv>0)), np.sum(np.abs(hv)*(np.abs(hv)>10**(-4)))])
        
        ranking = rnk(newpop_f,newpop_cv,EPSILON) 
        fevals = fevals + lam
...
# Computation (CEC), IEEE, 2018, https://dx.doi.org/10.1109/CEC.2018.8477950
    ...
    return gb_y, gb_fv, gb_cv, dyn_fit, dyn_cv, dyn_sig, dyn_ynorm, dyn_gen, dyn_fev
